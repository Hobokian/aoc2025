...
for battery_bank in input:
    ...
    for index in range(len(battery_bank)-1):
        ...
        if battery_bank[index] > battery_bank[index+1] and str(first_number) < battery_bank[index]:
    ...
    for indextwo in range(first_number_index+1, len(battery_bank)):
        print(battery_bank[first_number_index+indextwo])
        try:
            if battery_bank[first_number_index+indextwo] > battery_bank[first_number_index+indextwo+1] and str(second_number) < battery_bank[first_number_index+indextwo]:
                second_number = battery_bank[first_number_index+indextwo]
        except IndexError:
            continue
